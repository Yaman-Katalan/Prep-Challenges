...
const objLat = (obj) => {
  let fName, lName;
  fName = obj.firstName.charAt(0).toUpperCase() + obj.firstName.slice(1);
  lName = obj.lastName.charAt(0).toUpperCase() + obj.lastName.slice(1);
...
const cvFormatter = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].yearsOfExperience > 1) {
      let name;
      if (arr[i].firstName === null && arr[i].lastName === null) name = "";
      else if (arr[i].lastName === null) name = arr[i].firstName;
      else if (arr[i].firstName === null) name = arr[i].lastName;
      else name = `${arr[i].firstName} ${arr[i].lastName}`;
      newArray.push({
        fullName: name,
        tech: arr[i].tech,
      });
